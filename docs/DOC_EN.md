...
`CatChatScreen` switches screens with two booleans (`showSettings` / `showMemory`), returning early on a
match. Both use `rememberSaveable`: rotating recreates the activity, and if a screen is closed as a side
effect, the `rememberSaveable` drafts inside it leave the composition with it, so saving them was pointless.
The project pulls in no navigation library; a new screen follows the same pattern or comes with a navigation
migration.

**Where state belongs across rotation (activity recreation)**, shared by 12.2–12.4:

- A draft the user has entered or chosen but not yet submitted always uses `rememberSaveable`; otherwise one
  rotation costs them the whole form.
- In-flight request state (`testing` / `loadingModels`) stays in `remember`: its coroutine hangs off
  `rememberCoroutineScope` and is cancelled by recreation, so restoring `true` would only leave a spinner
  that never stops.
- An `AlertDialog` is a separate window whose view tree the activity's `onSaveInstanceState` does not save,
  so drafts inside it are always lost on rotation. The draft must be hoisted into the screen and held with
  `rememberSaveable`, leaving the dialog itself stateless.
...
- Input bar: a leading "+" opens "choose from gallery / take photo", and a multiline field (≤ 5 lines, IME
  ...
  The typed text and the staged images are held in `rememberSaveable`, as are the screen's `showSettings` /
  `showMemory` / `previewImage`; see [12.1](#121-screen-switching) for why.
...
All inputs are local `rememberSaveable` drafts; only pressing **Save** writes back to the ViewModel. Drafts
survive rotation, but the in-flight `testing` / `loadingModels` flags are not restored
(see [12.1](#121-screen-switching)).
...
Adding and editing share one dialog. The dialog itself is stateless: the category / keyword / content drafts
are held by the screen in `rememberSaveable`; see [12.1](#121-screen-switching) for why.
