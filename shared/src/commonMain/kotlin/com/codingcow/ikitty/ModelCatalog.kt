...
data class NumberParam(
    ...
    fun format(value: Float): String = formatFixed(clamp(value), decimals)
 ...
 * 固定小数位的格式化。
 *
 * Kotlin/Native 没有 `String.format`，而这里需要的只是"保留 N 位小数"，
 * 不值得为它引一套跨平台格式化库。行为对齐原来的 `String.format(Locale.US, "%.Nf", v)`：
 * 只用 '.' 作小数点，四舍五入，负数保留符号。
 */
private fun formatFixed(value: Float, decimals: Int): String {
    ...
    val fraction = magnitude % factor
    val body = "$whole.${fraction.toString().padStart(decimals, '0')}"
